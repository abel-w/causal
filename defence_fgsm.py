...
if __name__ == '__main__':
    ...
    path_im = r"D:\pypro\data\samples_attack\fgsm"
    list_all0 = os.listdir(path_im)
    sam_all = []
    for i in list_all0:
        nm = i.split('@')[0].split('_')[1]
        if not nm in str(sam_all):
            sam_all.append(os.path.join(path_im, i))

    # sam_c = random.sample(sam_all, 500)
    sam_c = sam_all
    ...
    y_res_cal = []
    # list_y_res =[]
    for i in range(len(sam_c)):
        img = np.array(Image.open(sam_c[i]))
        img = img[:, :, 0:3]
